...
class Checkout_qr extends CI_Controller
{
    public $mollie;
    ...
    public function data()
    {
        ...
        if ($this->session->userdata('order_number')) {
            ...
            if ($data['order']->billing_comment)
                $data['billingComment'] = $data['order']->billing_comment;
        } else {
        ...
        if (!empty($this->session->userdata['keep_input_value']['billingComment']))
            $data['billingComment'] = $this->session->userdata['keep_input_value']['billingComment'];
        ...
        $this->load->view('layout/mobile/view_checkout_data', $data);
            ...
            if ($valid == 1) {
                ...
                if (!empty($this->cart->contents())) {
                    ...
                    foreach ($this->cart->contents() as $item) {
                        for ($i = 0; $i < $item['qty']; $i++) {
                            ...
                            $form_data_item[] = array(
                                ...
                                "item_currency_icon" => $store_check['currency_icon'],
                                "item_currency_code" => $store_check['currency_code'],
                                "item_lang_code" => $store_check['lang_code'],
                                "item_subtotal" => $item['price'], //$item['subtotal'],
                                "order_number" => $this->session->userdata('order_number') ?? $this->session->userdata('order_number_token'),
                                "security_number" => $this->session->userdata('security_number') ?? $this->session->userdata('security_number_token')
                            );
                    ...
                    $form_data = array(
                        ...
                        "store_currency_code" => $store_check['currency_code'],
                        "store_currency_icon" => $store_check['currency_icon'],
                        "store_lang_code" => $store_check['lang_code'],
                    ...
                    redirect(base_url('mobile/checkout/payment_sent'));
                } else {
                    redirect(base_url('mobile/shop'));
                }
            } else {
    ...
    public function payment_sent()
    {
        ...
        if (empty($this->session->userdata('payment_form')['billing_firstname'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The First Name field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['billing_lastname'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Last Name field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['billing_email'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Email field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['billing_phone'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Phone field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['billing_street'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Street field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['billing_postcode'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Postcode field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['billing_city'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The City field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['land_id'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The land id field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['land_name'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Land name field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['store_id'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Store id field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['store_name'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Store name field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['store_lang_code'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Store lang code field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['store_currency_icon'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Store currency icon field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['store_currency_code'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Store currency code field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['order_type'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Order type field is required.", "mobile/shop");
        } elseif (empty($this->session->userdata('payment_form')['order_number'])) {
            $valid = 0;
            $this->set_flashdata_redirect("The Order number field is required.", "mobile/shop");
        } else {
            $valid = 1;
        }
        ...
        if ($valid == 1) {
            ...
            if (!empty($this->cart->contents())) {
                $this->insert_order_data();
                    ...
                    $data['qrcode'] = $this->qrcode_url('https://irispartner.de/mobile/shop');
                    $this->load->view('layout/mobile/view_header',$data);
    ...
    public function payment_check_mollie()
    {
        ...
        if ($payment->status === "paid" || $payment->isPaid()) {
            ...
            $update_order = $this->Model_order->update_order($this->session->userdata('order_number_token'), $order_data);
    ...
    public function qrcode_url($url)
    {
